...
OPTIONS
  --language    Limit repositories to those where the primary coding  [string]
                language matches this value
  --limit       Limit the number of repositories returned by the API  [integer]
                to the value specified
  --path        Location where the repositories will be stored after  [string]
                cloning or updating
  --sleep       Sleep the specified number of seconds before issuing  [integer]
                the next clone/update command. This can be useful in
                mitigating anti-DDOS and rate-limiting mechanisms.
  --topic       Limit repositories to those where one of the topics   [string]
                matches this value
